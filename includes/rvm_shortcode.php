...
function rvm_map_shortcode( $attr ) {    // $attr manages the shortcode parameter - [rvm_map mapid="xxxx" width="xxxx"]
    ...
    if ( isset( $attr[ 'mapid' ] ) && is_numeric( $attr[ 'mapid' ] ) ) { // Check if mapid attr exists and if is numeric
        ...
        if ( !empty( $postid ) ) {
            ...
            $map_bg_color =  !empty ( $map_bg_color ) ? $map_bg_color : RVM_MAP_BG_COLOR ;
            $map_bg_selected_color =  !empty ( $map_bg_selected_color ) ? $map_bg_selected_color : RVM_MAP_BG_SELECTED_COLOR ;
            $map_border_color = !empty ( $map_border_color ) ? $map_border_color : RVM_MAP_BORDER_COLOR ;            
            $rvm_mbe_map_marker_bg_color = !empty ( $rvm_mbe_map_marker_bg_color ) ? $rvm_mbe_map_marker_bg_color : RVM_MAP_MARKER_BG_COLOR ;
            $rvm_mbe_map_marker_border_color = !empty ( $rvm_mbe_map_marker_border_color ) ? $rvm_mbe_map_marker_border_color : RVM_MAP_MARKER_BORDER_COLOR ;
            
            $rvm_mbe_map_marker_dim_min = !empty ( $rvm_mbe_map_marker_dim_min ) ? $rvm_mbe_map_marker_dim_min : RVM_DIM_MIN_VALUE ;
            $rvm_mbe_map_marker_dim_max = !empty ( $rvm_mbe_map_marker_dim_max ) ? $rvm_mbe_map_marker_dim_max : RVM_DIM_MAX_VALUE ;
            ...
            */
            ...
            && is_array( $marker_array_unserialized['rvm_marker_long_array'] ) ) {
                ...
                if ( $output_is_markers ) {// if exist at least one value in DB
                ...
                //if ( !empty( $marker_array_serialized['rvm_marker_dim'] ) ) {
                ...
                for( $i=0; $i < $rvm_marker_array_count; $i++ ) {          
                    ...
                    if( !empty( $marker_array_unserialized['rvm_marker_dim_array'][ $i ] ) ) {
                        ...
                        $rvm_is_dim_value_more_then_two++ ; // will use it later in javascript to check if use the dimension scale or not ( not if counter is lowen then 2 )
                        
                        $output_is_marker_dim = 1 ;// variable fo later reference in javascript
                        $output .= esc_attr( str_replace( ',', '', $marker_array_unserialized['rvm_marker_dim_array'][ $i ] ) ) ; //replace all comma occurencies with '': it will lead in a issue in javascript array
            ...
            //if ( !empty( $marker_array_unserialized['rvm_marker_popup'] ) ) {
                ...
                for( $i=0; $i < $rvm_marker_array_count; $i++ ) {
                    ...
                    if( !empty( $marker_array_unserialized['rvm_marker_popup_array'][ $i ] ) ) {
                        ...
                        $output .= '"' . esc_attr( $marker_array_unserialized['rvm_marker_popup_array'][ $i ] ) . '"' ;
                ...
                $output_marker_popup .= 'label.html( label.html() + "<br>" + marker_dimensions[code] + marker_br + marker_popup[code] );' ;
                $output_marker_popup .= '}' ;             
                $output .= '];' ;
                
             }
                
            //} //if ( !empty( $marker_array_unserialized['rvm_marker_name_array'] ) )
            ...
            */                 
            ...
            foreach ( $array_regions as $region ) {
                ...
                if( is_array( $regionsparams_array ) && !empty( $regionsparams_array ) ) {                        
                    ...
                    if( !empty( $regionsparams_array[ 'field_region_bg' ] ) ) {                       
                        ...
                        $output .= '"' . $region[ 1 ] . '" : ' ;                        
                        $output .= '"' . esc_attr( $regionsparams_array[ 'field_region_bg' ] ) . '",'  ;
                        $output_is_regionsparams = 1 ;// variable fo later reference in javascript
            ...
            if ( $output_is_regionsparams ) {// if exist at least one value in DB
            ...
            */    
            ...
            //if ( isset( $output_is_regionsparams ) && $output_is_regionsparams &&  !empty( $marker_array_unserialized['rvm_marker_name'] ) ){
            ...
            */                
            ...
            if ( isset( $output_is_regionsparams ) && $output_is_regionsparams ) {
            ...
            */ 
            ...
            if ( isset( $output_is_markers ) && $output_is_markers ) {
                ...
                if ( isset( $output_is_marker_dim ) && $output_is_marker_dim && $rvm_is_dim_value_more_then_two > 1 ) {
                        
                    $is_markers_series_comma = ','; 
                ...
                //if( $is_regions_series_comma || $is_markers_series_comma ) {  $output_markers = ',' ; }
            ...
            */
            ...
            if ( isset( $output_is_marker_popup ) &&  $output_is_marker_popup ) { $output .= $output_marker_popup ; }
            
            $output .= ', onMarkerClick: function(events, index) {' ; // markers links        
