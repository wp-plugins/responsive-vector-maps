(function($) {
	$(function() {	
		...
		$( '.rvm_tabs' ).click( function(event) {
			...
			//document.cookie = "rvm_active_tab=" + activeTab ;// set the active tab as a cookie
		});
		
		/*$( document ).ready(function() {//get the active tab cookie value
			document.cookie = "rvm_active_tab=rvm_main_settings_tab" ;// set the main setting as default cookie for active tab		
			function getCookie(rvm_active_tab) {
		    var active_tab = rvm_active_tab + "=" ;
		    var ca = document.cookie.split(';') ;
			    for(var i=0; i<ca.length; i++) {
			        var c = ca[i];
			        while (c.charAt(0)==' ') c = c.substring(1) ;
			        	if (c.indexOf(name) != -1) return c.substring(active_tab.length, c.length) ;
			    }
				    return "";
			}
			
			var active_tab = getCookie('rvm_active_tab') ;
			
			if( active_tab ) {
				
				alert(active_tab) ;
								
				$( '#'+active_tab).addClass( 'rvm_active' ) ;
				$( '#'+active_tab+'_tab' ).addClass( 'rvm_active' ) ;
