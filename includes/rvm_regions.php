...
foreach( $array_regions as $region ) {
    ...
    $output .= '<td><p><input ' . RVM_REGION_LINK_CLASS . ' type="text" name="' . $region[ 1 ]  . '[]" value="' . esc_url( $regionsparams_array[ 'field_region_link' ] ) . '" ></p></td>' ; 
    $output .= '<td><p><input class="rvm_color_picker" type="text" name="' . $region[ 1 ]  . '[]" value="' . strip_tags( $regionsparams_array[ 'field_region_bg' ] ) . '" ></p></td>' ;
    ...
    $output .= '<td><p><textarea rows="1" name="' . $region[ 1 ]  . '[]" >' . esc_attr( wp_unslash( $regionsparams_array[ 'field_region_popup' ] ) ) . '</textarea></p></td>' ;
    $output .= '</tr>' ;
