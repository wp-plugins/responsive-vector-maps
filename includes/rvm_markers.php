...
if( is_array( $marker_array_unserialized[ 'rvm_marker_name_array' ] ) && $rvm_marker_array_count > 0  ) {            
    ...
    for( $i=0; $i < $rvm_marker_array_count; $i++ ) {
        ...
        $output .= '<p><label for="marker_name" ' . RVM_LABEL_CLASS . '>' . __( 'Name' , RVM_TEXT_DOMAIN ) . '*</label><input type="text" name="rvm_marker_name[]" value="' . esc_attr( $marker_array_unserialized[ 'rvm_marker_name_array' ][ $i ] ) . '" /></p>' ;            
        $output .= '<p><label for="marker_lat" ' . RVM_LABEL_CLASS . '>' . __( 'Latitude' , RVM_TEXT_DOMAIN ) . '*</label><input type="text" name="rvm_marker_lat[]" value="' . esc_attr( $marker_array_unserialized[ 'rvm_marker_lat_array' ][ $i ] ) . '" placeholder="e.g. 48.921537" /></p>' ;            
        $output .= '<p><label for="marker_long" ' . RVM_LABEL_CLASS . '>' . __( 'Longitude' , RVM_TEXT_DOMAIN ) . '*</label><input type="text" name="rvm_marker_long[]" value="' . esc_attr( $marker_array_unserialized[ 'rvm_marker_long_array' ][ $i ] ) . '" placeholder="e.g. -66.829834" /></p>' ;       
