...
function rvm_fields_array() {
    ...
    $fields[ 'rvm_mbe_map_bg_selected_color' ] = array( 'rvm_mbe_map_bg_selected_color', 'text', __( 'Selected Map Color', RVM_TEXT_DOMAIN ) , '', 7, 7, 1, 'main' ) ;
    ...
    $fields[ 'rvm_mbe_marker_count' ] = array( 'rvm_mbe_marker_count', 'hidden', '' , '', '', '', 1, 'main' ) ;
    ...
    $fields[ 'rvm_mbe_map_marker_dim_min' ] = array( 'rvm_mbe_map_marker_dim_min', 'select', __( 'Minimum value' , RVM_TEXT_DOMAIN ) , '', '', 10, 1, 'main' ) ;
    $fields[ 'rvm_mbe_map_marker_dim_max' ] = array( 'rvm_mbe_map_marker_dim_max', 'select', __( 'Maximum value' , RVM_TEXT_DOMAIN ) , '', '', 10, 1, 'main' ) ;        
...
function rvm_mb_function( $post ) {
    ...
    $output .= '<div id="rvm_tabs"><ul><li id="rvm_main_settings_tab" class="rvm_tabs rvm_active" rel="rvm_main_settings"><a href="#">Main settings</a></li>' ;
    
    if ( !empty( $rvm_selected_map ) ) {      
        $output .= '<li id="rvm_regions_countries_tab" class="rvm_tabs" rel="rvm_regions_countries"><a href="#">Regions/Countries</a></li>' ;
        $output .= '<li id="rvm_markers_tab" class="rvm_tabs" rel="rvm_markers"><a href="#">Markers</a></li>' ;
    }
    ...
    $output .= '<div id="rvm_main_settings" class="rvm_active hidden">';
    ...
    foreach ( $array_fields as $field ) {
        ...
        if ( $field[ 1 ] == 'text' ) {
            ...
            if ( empty( $field_value ) && $field[ 0 ] == 'rvm_mbe_map_bg_selected_color' ) { $field_value = RVM_MAP_BG_SELECTED_COLOR ; }
            if ( empty( $field_value ) && $field[ 0 ] == 'rvm_mbe_map_bordercolor' ) { $field_value = RVM_MAP_BORDER_COLOR ; }
                            
            if ( $field[ 7 ] == 'markers' && ( $field[ 0 ] == 'rvm_mbe_map_marker_bg_color' || $field[ 0 ] == 'rvm_mbe_map_marker_border_color' ) ) {                

                if ( empty( $field_value ) && $field[ 0 ] == 'rvm_mbe_map_marker_bg_color' ) { $field_value = RVM_MAP_MARKER_BG_COLOR ; }
                if ( empty( $field_value ) && $field[ 0 ] == 'rvm_mbe_map_marker_border_color' ) { $field_value = RVM_MAP_MARKER_BORDER_COLOR ; }
                $output_markers_value .=  '<div class="rvm_markers_values">' ;
                $output_markers_value .= '<label for="' . $field[ 0 ] . '" ' . RVM_LABEL_CLASS . '>' . $field[ 2 ] . '</label>' ;           
                $output_markers_value .= '<input ' . $id_and_class . ' type="' . $field[ 1 ] . '" name="' . $field[ 0 ] . '" value="' . esc_attr( $field_value ) . '" ' ;            
                if ( !empty( $field[ 4 ] ) ) { $output_markers_value .= ' maxlength="'. $field[ 4 ]. '" '; }
                if ( !empty( $field[ 5 ] ) ) { $output_markers_value .= ' size="' . $field[ 5 ] . '" '; }
                $output_markers_value .= ' />' ;
                $output_markers_value .=  '</div>' ;
        ...
        if ( $field[ 1 ] == 'select') {
            ...
            else if ( $field[ 0 ] == 'rvm_mbe_map_marker_dim_min' ) {
                
                $output_markers_dim_value .= '<p id="rvm_dim_min_value_wrapper"><label for="' . $field[ 0 ] .'" ' . RVM_LABEL_CLASS . '>' . $field[ 2 ] . '</label>' ;
                $output_markers_dim_value .= '<select  ' . $id_and_class . ' name="' . $field[ 0 ] . '" id="' . $field[ 0 ] . '">' ;
                                
                for ( $i=1; $i< 21; $i++ ) {
                        
                    //if ( empty( $field_value ) ) { $selected_value = RVM_DIM_MIN_VALUE ; } else { $selected_value = && $i === RVM_DIM_MIN_VALUE  ; }
                    
                    $field_value = empty( $field_value ) ? RVM_DIM_MIN_VALUE : $field_value ;
                    $output_markers_dim_value .= '<option ' . selected( $i, $field_value, false ) . ' >' . $i . '</option>' ;
                }
                
                $output_markers_dim_value .= '</select></p>' ;
                
            }

            else if ( $field[ 0 ] == 'rvm_mbe_map_marker_dim_max' ) {
                
                $output_markers_dim_value .= '<p id="rvm_dim_max_value_wrapper"><label for="' . $field[ 0 ] .'" ' . RVM_LABEL_CLASS . '>' . $field[ 2 ] . '</label>' ;
                $output_markers_dim_value .= '<select  ' . $id_and_class . ' name="' . $field[ 0 ] . '" id="' . $field[ 0 ] . '">' ;
                
                for ( $i=1; $i< 21; $i++ ) {
                        
                    //if ( $i === RVM_DIM_MAX_VALUE  ) { $selected_value = "selected='selected'" ; } else { $selected_value = '' ; }
                    $field_value = empty( $field_value ) ? RVM_DIM_MAX_VALUE : $field_value ;
                    $output_markers_dim_value .= '<option ' . selected( $i, $field_value, false ) .  ' >' . $i . '</option>' ;
                }
        ...
        if ( $field[ 1 ] == 'hidden' ) {
            ...
            if ( $field[ 0 ] == 'rvm_mbe_marker_count' ) {
    ...
    if ( isset( $_POST[ 'rvm_mbe_select_map' ] ) && $_POST[ 'rvm_mbe_select_map' ] != 'select_country'  ) {
        ...
        foreach ( $array_fields as $field ) {
            ...
            if ( $field[ 0 ] == 'rvm_mbe_zoom' ) {
                ...
                //check if minimum value is not bigger then the maximum value
                if ( $_POST[ 'rvm_mbe_map_marker_dim_min' ] > $_POST[ 'rvm_mbe_map_marker_dim_max' ] ) {
                    
                    $_POST[ 'rvm_mbe_map_marker_dim_min' ] = RVM_DIM_MIN_VALUE ;
                    $_POST[ 'rvm_mbe_map_marker_dim_max' ]  = RVM_DIM_MAX_VALUE ;
        ...
        if ( isset( $_POST[ 'rvm_marker_name' ] ) ) {
            ...
            if ( !empty( $_POST[ 'rvm_marker_name' ] ) ) {
                ...
                $rvm_marker_name_serialize = serialize( $_POST[ 'rvm_marker_name' ] ) ;            
                update_post_meta( $post_id, '_rvm_marker_name', strip_tags( $rvm_marker_name_serialize ) ) ;
                
            } else { update_post_meta( $post_id, '_rvm_marker_name', '' ) ; }
            
            if ( !empty( $_POST[ 'rvm_marker_lat' ] ) ) {
                
                $rvm_marker_lat_array = check_is_number_in_array( $_POST[ 'rvm_marker_lat' ] ) ;//check if is valid entry          
                ...
                update_post_meta( $post_id, '_rvm_marker_lat', strip_tags( $rvm_marker_lat_serialize ) ) ;
                
            } else { update_post_meta( $post_id, '_rvm_marker_lat', '' ) ; }        
            
            if ( !empty( $_POST[ 'rvm_marker_long' ] ) ) {                
                
                $rvm_marker_long_array = check_is_number_in_array( $_POST[ 'rvm_marker_long' ] ) ; //check if is valid entry                
                           
                $rvm_marker_long_serialize = serialize( $rvm_marker_long_array ) ;
                $rvm_marker_long_serialize = str_replace( ',', '.', $rvm_marker_long_serialize ) ;
                update_post_meta( $post_id, '_rvm_marker_long', strip_tags( $rvm_marker_long_serialize ) ) ;
            ...
            if ( !empty( $_POST[ 'rvm_marker_link' ] ) ) {
                ...
                update_post_meta( $post_id, '_rvm_marker_link', strip_tags( $rvm_marker_link_serialize ) ) ;
                
            }  else { update_post_meta( $post_id, '_rvm_marker_link', '' ) ; }
            
            if ( !empty( $_POST[ 'rvm_marker_dim' ] ) ) {
                
                $rvm_marker_dim_array = check_is_number_in_array( $_POST[ 'rvm_marker_dim' ] ) ; //check if is valid entry                
               
                $rvm_marker_dim_serialize = serialize( $rvm_marker_dim_array ) ;
                $rvm_marker_dim_serialize = str_replace( ',', '.', $rvm_marker_dim_serialize ) ;
                update_post_meta( $post_id, '_rvm_marker_dim', strip_tags( $rvm_marker_dim_serialize ) ) ;
                
            }  else { update_post_meta( $post_id, '_rvm_marker_dim', '' ) ; }
    
            if ( !empty( $_POST[ 'rvm_marker_popup' ] ) ) {
                
                $rvm_marker_popup_serialize = serialize( $_POST[ 'rvm_marker_popup' ] ) ;
                update_post_meta( $post_id, '_rvm_marker_popup', strip_tags( $rvm_marker_popup_serialize ) ) ;
                
            }  else { update_post_meta( $post_id, '_rvm_marker_popup', '' ) ; }

        } //if ( isset( $_POST[ 'rvm_marker_name' ] ) )
        
        else { // if nothing is sent reset all data -  change with delete_post_meta_by_key
...
function rvm_ajax_preview( $post_id ) {    
    ...
    if( isset( $_REQUEST[ 'nonce' ] ) && isset( $_REQUEST[ 'map' ] ) && $_REQUEST[ 'map' ] != 'select_country' ) {
        ...
        if( wp_verify_nonce( $_REQUEST[ 'nonce' ] , 'rvm_ajax_nonce' ) ) {
            ...
            $map_bg_color = !empty( $_REQUEST[ 'bgcolor' ] ) ? $_REQUEST[ 'bgcolor' ]  : RVM_MAP_BG_COLOR ;
            $map_bg_selected_color = !empty( $_REQUEST[ 'bgselectedcolor' ] ) ? $_REQUEST[ 'bgselectedcolor' ]  : RVM_MAP_BG_SELECTED_COLOR ;
